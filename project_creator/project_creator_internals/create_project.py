...
class ClassBasedProject(CreatorInternals, CssHelpers, HtmlHelpers, AdminHelpers):

    def create_project(self, project_location=None, apps_folder_location=None,
                       parent_directory=None, config_folder="config",
                       project_name=None, **app_names_and_pages):

        """creates a skeleton for a flask project and an initial app called main """

        self._make_initial_directories(project_location, project_name,
                                       apps_folder_location, parent_directory,
                                       **app_names_and_pages)
        self._make_initial_files(project_location, apps_folder_location,
                                 parent_directory, project_name,
                                 **app_names_and_pages)
    ...
    def add_page(self, project_name=None, project_location=None,
        ...
        try:
            os.remove(url_rules_path(app))
            os.remove(views_path(app))
    ...
    def add_app(self, project_location=None, project_name=None,
        ...
        for app, pages in app_names_and_pages.items():
            ...
            self._create_new_css_folders(project_location, project_name, app)
            self._create_new_html_folders(project_location, project_name, app)
            self._create_forms_and_models(apps_folder_location, app)
            self._create_contents_of_static_folder(project_location, project_name, app)
            create_file(file_path=(apps_folder_location, app,
                                   "__init__.py"),
            ...
            for page in pages:
                ...
                self._create_new_css_files(project_location, project_name,
                                           app, page)
                self._create_new_html_files(project_location,
                                            project_name, app, page)
            for page in pages:
